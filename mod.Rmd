...
compare_lmm_rf <- function(var, rf_model, lmm_model, train_data) {
  # Partial Dependence Plot from Random Forest
  pd <- partial(rf_model, pred.var = var, train = train_data)
  p_rf <- autoplot(pd, rug = TRUE, train = train_data) +
    labs(title = paste("Random Forest PDP -", var),
         x = var, y = "Predicted Math Score") +
    theme_minimal(base_size = 12) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))

  # Effect Plot from Linear Mixed Model
  ef <- Effect(var, lmm_model)
  p_lmm <- plot(ef, main = paste("LMM Effect -", var), ylab = "Predicted Math Score")

  # Show both side-by-side
  grid.arrange(p_rf, p_lmm, ncol = 2)
}
compare_lmm_rf("read", rf_model, model_lmer_full, train_data)
compare_lmm_rf("escs", rf_model, model_lmer_full, train_data)
compare_lmm_rf("book", rf_model, model_lmer_full, train_data)
